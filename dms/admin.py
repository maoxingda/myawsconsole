...
from dms.models import ReplicationEndpoint, ReplicationTask, Table, Task
...
class ReplicationTaskAdmin(admin.ModelAdmin):
    ...
    search_fields = ("replication_task_identifier",)
    date_hierarchy = "replication_task_creation_date"
    ordering = ("replication_task_identifier",)
    list_display = (
...
class ReplicationEndpointAdmin(admin.ModelAdmin):
    ...
    def html_actions(self, obj):
        ...
        if obj.endpoint_type == "SOURCE":
            url = reverse("admin:dms_replicationtask_changelist") + f"?source_endpoint_id={obj.id}"
        else:
            url = reverse("admin:dms_replicationtask_changelist") + f"?target_endpoint_id={obj.id}"
