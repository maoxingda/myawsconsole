...
from dms.models import Endpoint, ReplicationEndpoint, ReplicationTask, Table, Task
...
def refresh_tasks(request):
    ...
    return redirect(
        reverse(f"admin:{'_'.join(request.path.split('/')[1:3])}_changelist") + f"?q={endpoint_arn}{table_name}"
    )
...
def refresh_endpoints(request):
    ...
    local_endpoint_identifiers = {endpoint.identifier for endpoint in Endpoint.objects.filter(server_name=server_name)}
...
def refresh_tables(request, task_id):
    ...
    for page in paginator.paginate(ReplicationTaskArn=task.arn):
        for stat in page["TableStatistics"]:
            tables.add(
                (
                    partition_table_name_suffix_pattern.sub("", stat["TableName"]),
                    stat["SchemaName"],
                )
            )

    tables = [
        Table(name=table[0], schema=table[1], task_name=task.name, task=task)
        for table in tables
...
def download_table_mapping(request, task_id):
    ...
    return JsonResponse({"name": task.name, "table_mappings": json.dumps(task.table_mappings, indent=2)})
