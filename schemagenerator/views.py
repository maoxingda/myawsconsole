...
def db_tables(request, conn_id):
    ...
    if db_conn.db_type == DbConn.DbType.POSTGRESQL.value:
        with psycopg2.connect(db_conn.server_address()) as conn:  # noqa: SIM117
    ...
    return redirect(
        reverse(f"admin:{'_'.join(request.path.split('/')[1:3])}_changelist") + "?conn__id__exact=" + str(conn_id)
    )
...
def launch_task(request, task_id):
    ...
    def do_task():
        try:
            client = boto3.client("dms")
            endpoint_id = f"{task.conn.name}-{task.name}-sync-{task.task_type}-{settings.ENDPOINT_SUFFIX}".replace(
                "_", "-"
            )  # 不能包含下划线
            ...
            if pwd.getpwuid(os.getuid())[0] != "ec2-user":
                response = client.describe_endpoints(
                    Filters=[{"Name": "endpoint-id", "Values": ["bi-sandbox-acceptanydate"]}]
                )
                    ...
                    Filters=[
                        {
                            ...
                            "Values": ["bi-sandbox-private-replication-instance"],
                        }
                    ]
                )
                ...
                response = client.describe_replication_instances(
                    Filters=[
                        {
                            "Name": "replication-instance-id",
                            "Values": ["hc-replica-server-private-v3"],
                        }
                    ]
                )
                replication_instance_arn = response["ReplicationInstances"][0]["ReplicationInstanceArn"]
...
def create_ddl_sql(request, task_id):
    ...
    with psycopg2.connect(dns) as conn:  # noqa: SIM117
        with conn.cursor() as cursor:
            ...
            for sync_table in task.sync_tables.all():
                ...
                ddl_sql += (
                    f"create external table emr.{task.conn.target_table_name_prefix.replace('init_', '')}{sync_table.table.name}\n"  # noqa: E501
                    f"(\n"
                )
                for (
                    column,
                    ...
                    if data_type in (
                        ...
                        ddl_sql += (
                            f"    {column.ljust(max_column_lengh)} {data_type}({numeric_precision}, {numeric_scale}),\n"  # noqa: E501
                        )
                        ...
                        messages.error(
                            request,
                            mark_safe(
                                f"<pre>未知数据类型：{task.conn.target_schema, sync_table.table.name, column, data_type}</pre>"  # noqa: E501
                            ),
                        )
    ...
    return JsonResponse({"ddl_sql": ddl_sql})
...
def slot(request, pk):
    ...
    if dbconn.db_type == DbConn.DbType.POSTGRESQL.value:
        with psycopg2.connect(dbconn.server_address()) as conn:  # noqa: SIM117
            with conn.cursor() as cursor:
                cursor.execute(
                    "select database, active, pg_size_pretty( pg_wal_lsn_diff(pg_current_wal_lsn(), restart_lsn) ), slot_name as lag from pg_replication_slots order by 1"  # noqa: E501
                )
